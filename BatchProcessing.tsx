...
export const BatchProcessing = ({ onBack, className }: BatchProcessingProps) => {
  ...
  // Batch processing logic in useEffect
  useEffect(() => {
    let timer: NodeJS.Timeout;
    const processNextItem = async () => {
      if (!isProcessing || currentProcessingIndex >= batchItems.length) return;
    ...
    if (isProcessing && currentProcessingIndex < batchItems.length) {
      timer = setTimeout(processNextItem, 100);
    } else if (isProcessing && currentProcessingIndex >= batchItems.length) {
      setIsProcessing(false);
      toast({
  ...
  }, [isProcessing, currentProcessingIndex, batchItems.length, toast]);
        ...
        {batchItems.length > 0 && (
          <>
            {!isProcessing ? (
              ...
              <Button onClick={() => setIsProcessing(false)} variant="warning" size="mobile">
